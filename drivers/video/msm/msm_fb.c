...
//#ifdef CONFIG_MSM_HDMI
#if 0
	u32 len = PAGE_ALIGN((start & ~PAGE_MASK) + v_smem_len);   //info->fix.smem_len);
#else
...
//#ifdef CONFIG_MSM_HDMI
#if 0
		if(800 == panel_info->xres)
		{
			v_smem_len = (ALIGN(v_xres, 32)) * (bpp)  * (v_yres) * (mfd->fb_page) ;
		}
#endif
#endif
...
//#ifdef CONFIG_MSM_HDMI
#if 0
	if( panel_info->xres == 800 )
	{
		fbram += v_smem_len;
		fbram_phys += v_smem_len;
		fbram_size -= v_smem_len;
	}
	else
	{
		fbram += fix->smem_len;
